...
harden_sshd_config() {
  if [[ ! $(sudo grep travis /etc/ssh/sshd_config) ]]; then
...
EOF
    ...
    echo "--- sshd config already hardened. Skipping."
  fi
}

dot_bashrc_setup() {
  if [[ ! $(grep travis ~/.bashrc) ]]; then
...
EOF
    ...
    echo "--- .bashrc is good. Skipping."
  fi
}

dot_profile_setup() {
  if [[ ! $(grep travis ~/.profile) ]]; then
...
EOF
    ...
    echo "--- .profile is good. Skipping."
  fi
}

dot_bash_profile_setup() {
  if [[ ! $(grep travis ~/.bash_profile) ]]; then
...
EOF
  ...
  source ~/.bash_profile
    ...
    echo "--- .bash_profile is good. Skipping."
  fi
}
...
set_env_travisci_true() {
  if [[ ! $(grep TRAVIS /etc/launchd.conf) ]]; then
...
EOF
   ...
   echo '--- global env variables already set in /etc/launchd.conf. Skipping.'
  fi
}
